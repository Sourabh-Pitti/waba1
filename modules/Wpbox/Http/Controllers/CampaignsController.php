...
class CampaignsController extends Controller
{
    ...
    public function report(Campaign $campaign){
        ...
        fputcsv($handle, array('Name', 'Phone', 'Country', 'Status', 'Sent at', 'Delivered at','Read at','Extra'));
        foreach ($campaign->messages as $key => $message) {
            ...
            fputcsv($handle, array($message->contact->name, $message->contact->phone, $message->contact->country->name, $status, $message->scchuduled_at?$message->scchuduled_at:$message->created_at, $message->delivered_at, $message->read_at,$error));
        }
        fclose($handle);
        $headers = array(
